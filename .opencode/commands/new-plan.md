...
3. **Create the plan file**
   ...
   **Formatting rule**: Write all prose as single unwrapped lines per paragraph — no hard line breaks mid-sentence. Break only at blank lines (paragraph separators), list item boundaries, headings, and table rows. Never wrap prose at a column width.
